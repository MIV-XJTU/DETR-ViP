...
from ..fusion_layer import SelectiveBiAttentionBlock, EfficientSelectiveBiAttentionBlock
try:
...
class LiteDETRViPTransformerEncoder(DeformableDetrTransformerEncoder):
    ...
    def _init_layers(self) -> None:
            ...
            if self.num_expansion == self.num_fusion:
                ...
                for i in range(self.num_layers):
                    if (i + 1) % (self.num_layers / self.num_expansion) == 0:
                        fusion_layers.append(SelectiveBiAttentionBlock(**self.fusion_layer_cfg))
                    else:
                        fusion_layers.append(None)
                self.fusion_layers = ModuleList(fusion_layers)
            else:
        ...
        if self.num_expansion == self.num_fusion:
            memory_trans_fc_list = []
            memory_trans_norm_list = []
            for i in range(self.num_layers):
                if (i + 1) % (self.num_layers / self.num_expansion) == 0:
                    memory_trans_fc_list.append(copy.deepcopy(memory_trans_fc))
                    memory_trans_norm_list.append(copy.deepcopy(memory_trans_norm))
                else:
                    memory_trans_fc_list.append(None)
                    memory_trans_norm_list.append(None)
                self.memory_trans_fc = ModuleList(memory_trans_fc_list)
                self.memory_trans_norm = ModuleList(memory_trans_norm_list)
        else:
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if (layer_id + 1) % (self.num_layers / self.num_expansion) == 0:
                ...
                if self.fusion_layers[layer_id]:
                    ...
                    #     visual_feature=tgt,
                    #     lang_feature=memory_prompt,
                    #     attention_mask_v=key_padding_mask[:, level_start_index[4-self.encoder_scale]:] if key_padding_mask is not None else key_padding_mask,
                    #     attention_mask_l=prompt_attention_mask,
                    #     adaptive_mask=fuse_masks,
                    #     prompt_mode=prompt_mode
                    #     # attention_mask_l=prompt_attention_mask,
